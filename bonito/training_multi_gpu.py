...
def train(model, device, train_loader, optimizer, use_amp=False):
    ...
    with progress_bar:
        ...
        for data, out_lengths, target, lengths in train_loader:
            ...
            loss = criterion(log_probs.transpose(0, 1), target, out_lengths / model.module.stride, lengths)
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (data, out_lengths, target, lengths) in enumerate(test_loader, start=1):
            ...
            log_probs = model(data)
            test_loss += criterion(log_probs.transpose(1, 0), target, out_lengths / model.module.stride, lengths)
            predictions.append(torch.exp(log_probs).cpu())
            prediction_lengths.append(out_lengths / model.module.stride)

    predictions = np.concatenate(predictions)
    lengths = np.concatenate(prediction_lengths)

    references = [decode_ref(target, model.module.alphabet) for target in test_loader.dataset.targets]
    sequences = [decode(post[:n], model.module.alphabet) for post, n in zip(predictions, lengths)]
